...
<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-8 mt-10">
        ...
        <form action="{{ route('invoices.update', $invoice->id) }}" method="POST">
                ...
                <div id="products" class="space-y-4">
                    @foreach($invoice->products as $index => $product)
                        <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 product-item">
                            <select name="products[{{ $index }}][id]" class="product-select w-full sm:w-2/3 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <input type="number" name="products[{{ $index }}][quantity]" value="{{ $product->pivot->quantity }}" placeholder="Quantité" min="1" class="product-quantity w-full sm:w-1/3 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div id="soins" class="space-y-4">
                    @foreach($invoice->soins as $index => $soin)
                        <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 soin-item">
                            <select name="soins[{{ $index }}][id]" class="soin-select w-full sm:w-2/3 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <input type="number" name="soins[{{ $index }}][quantity]" value="{{ $soin->pivot->quantity }}" placeholder="Quantité" min="1" class="soin-quantity w-full sm:w-1/3 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="mb-6 mt-6">
                ...
                <input type="text" id="total_amount" value="{{ $invoice->total_amount }}" class="block w-full bg-gray-100 border border-gray-300 rounded-lg shadow-sm p-3" readonly>
            </div>
    ...
    <script>
        // Use the same dynamic logic for adding/removing products/soins as in the create blade
        let productIndex = {{ $invoice->products->count() }};
        let soinIndex = {{ $invoice->soins->count() }};
